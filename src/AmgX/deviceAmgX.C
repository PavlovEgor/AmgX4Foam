...
namespace Foam
{
    defineTypeNameAndDebug(deviceAmgX, 0);

    devicelduMatrix::solver::addsymMatrixConstructorToTable<deviceAmgX>
        adddeviceAmgXSymMatrixConstructorToTable_;

    devicelduMatrix::solver::addasymMatrixConstructorToTable<deviceAmgX>
        adddeviceAmgXAsymMatrixConstructorToTable_;
}

Foam::word Foam::deviceAmgX::matType_ = "NA";


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::deviceAmgX::deviceAmgX
(
...
void Foam::deviceAmgX::buildAndApplyMatrixPermutation
(
...
//- construct the matrix for AmgX
void Foam::deviceAmgX::applyMatrixPermutation
(
    deviceCsrMatrix* csrMatrix,
    label& nRowsGlobal
) const
...
Foam::solverPerformance Foam::deviceAmgX::solve
(
...
{
    const fvMesh& fvm = dynamicCast<const fvMesh>(this->matrix_.mesh().openFoamMesh().thisDb());
    
    label nCells = psi.size();

    const linearSolverContextTable<AmgXLinearSolverContext<matrix>>& contexts =
        linearSolverContextTable<AmgXLinearSolverContext<matrix>>::New(fvm);

    AmgXLinearSolverContext<matrix>& ctx = contexts.getContext(eqName_);
    ...
    ctx.performance = solPerf;

    AmgXSolver& amgx = ctx.amgx_;
    
    matrix& Amat = ctx.Amat_;
...
) const
{
    ...
    if(matType_ == "NA")
    {
        matType_ = this->matrix_.mesh().openFoamMesh().solverDict(eqName_).get<word>("matrixType");
    }

    if(matType_ == "CSR")
    {
        return solve<deviceCsrMatrix>(solverPerf, psi, source);
    }
    else if(matType_ == "MSR")
    {
        return solve<deviceMsrMatrix>(solverPerf, psi, source);
    }
    else
    {
        FatalErrorInFunction << "Invilid matrix type for Amgx. Allowed are: CSR, MSR." 
            << nl << abort(FatalError);
